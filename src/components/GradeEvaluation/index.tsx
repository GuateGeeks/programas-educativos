...
export default function GradeEvaluation({moduleId, grade: controlledGrade, onGradeChange}: GradeEvaluationProps): React.JSX.Element {
  ...
  const modeLabel = assessment?.mode === 'required' ? 'Evaluación requerida' : assessment?.mode === 'optional' ? 'Referencia opcional' : 'Observación de proceso';
  if (!assessment || !selected) return <section className={styles.panel}><p>No hay una referencia de evaluación configurada para este módulo.</p></section>;

  return (
    <section className={styles.panel} aria-label="Evaluación por contexto">
      <div className={styles.top}>
        <div><span>Evaluación del módulo</span><h3>Referencia para observar al equipo</h3><p>Seleccione el contexto que desea acompañar. No es una calificación individual.</p></div>
        <label>Grado<select value={grade} onChange={(event) => changeGrade(event.target.value as GradeId)}>{GRADES.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label>
      </div>
      <div className={styles.contextNav} role="tablist" aria-label="Ruta de evaluación">
        <button type="button" role="tab" aria-selected={route === 'guided'} className={route === 'guided' ? styles.active : ''} onClick={() => changeRoute('guided')}><b>01</b><span><strong>Ruta guiada</strong><small>Construir y comprender</small></span></button>
        <button type="button" role="tab" aria-selected={route === 'open'} className={route === 'open' ? styles.active : ''} onClick={() => changeRoute('open')}><b>02</b><span><strong>Desafío abierto</strong><small>Diseñar y resolver</small></span></button>
      </div>
      <div className={styles.selectionGrid}>
        <label>Sesión<select value={selected.id} onChange={(event) => {setSessionId(event.target.value); setBlockIndex(0);}}>{routeSessions.map((session) => <option key={session.id} value={session.id}>{session.title}</option>)}</select></label>
        <label>Bloque<select value={blockIndex} onChange={(event) => setBlockIndex(Number(event.target.value))}>{(selected.blocks || []).map((block, index) => <option key={block.id} value={index}>{index + 1}. {block.title}</option>)}</select></label>
      </div>
      ...
      {assessment.mode === 'observation' && <div className={styles.observationNotice}><strong>Observe y converse</strong><span>Este momento no requiere una puntuación. Use los criterios para orientar el siguiente paso del equipo.</span></div>}
      ...
      <div className={styles.nextStep}><b>Siguiente paso</b><span>{assessment.nextStep}</span></div>
    </section>
  );
}
