...
// Import-time integrity check, mirroring the GuateGeeks and CiudadBots data
// layers. Runs during SSR/build (and in the browser), so a malformed session
// set fails the build instead of shipping silently. Compile-time, the
...
(function validateSessions(): void {
  if (sessions.length !== EXPECTED_SESSION_COUNT) {
    throw new Error(
      `Tiempo Circular: expected ${EXPECTED_SESSION_COUNT} sessions, found ${sessions.length}.`,
    );
  }
  ...
  for (const [i, s] of sessions.entries()) {
    if (!s.id || seenIds.has(s.id)) {
      throw new Error(`Tiempo Circular: invalid or duplicate session id: "${s.id}".`);
    }
    seenIds.add(s.id);

    if (!s.slug.trim()) {
      throw new Error(`Tiempo Circular: session "${s.id}" has no slug.`);
    }
    if (seenSlugs.has(s.slug)) {
      throw new Error(`Tiempo Circular: duplicate session slug: "${s.slug}".`);
    }
    seenSlugs.add(s.slug);

    if (s.phaseKinds.length !== 4) {
      throw new Error(
        `Tiempo Circular: session "${s.id}" must have 4 phase kinds, has ${s.phaseKinds.length}.`,
      );
    }
    ...
    if (s.n !== expectedN) {
      throw new Error(
        `Tiempo Circular: session "${s.id}" is at position ${i + 1} but is numbered "${s.n}".`,
      );
    }
  }
  ...
  for (let i = 1; i < sessions.length; i++) {
    if (sessions[i].retoLevel < sessions[i - 1].retoLevel) {
      throw new Error(
        `Tiempo Circular: reto level regresses from session "${sessions[i - 1].id}" ` +
