...
Defines the GuateGeeks Liquid Glass design system: the single theming contract for the Docusaurus hub and all first-party program components. Covers the coral/sky/plum brand palette and glass surface tokens (light and dark mode), reusable glass utility classes, Barlow-based brand typography, Docusaurus chrome theming (navbar, sidebar, buttons, footer, focus ring, reduced motion), the swizzled sidebar theme components that enable a hideable sidebar, and the requirement that program components consume these shared tokens rather than defining a per-program palette.
