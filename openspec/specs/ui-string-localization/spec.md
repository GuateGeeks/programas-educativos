...
Defines the requirement that all UI-chrome strings — React component text, the student build view, and site navbar/footer labels — are authored through Docusaurus's translation APIs rather than hardcoded, so no Spanish text leaks into the English locale.
